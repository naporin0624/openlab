    ...
    <v-container>
      <v-layout wrap align-center justify-center row fill-height>
        <v-flex md6 xs12 ref="videobox">
          ...
          <v-container>
            <v-layout wrap align-center justify-center row fill-height>
              <v-flex md4 v-for="(img1, idx) in personImage" :key="img1">
                <img
                  :src="img1"
                  width="160"
                  height="160"
      ...
      if (!this.isFaceRecognitionStart) clearInterval();
        ...
        if (context) {
          ...
          const datasets: Chart.ChartDataSets[] = predicts.map(
            (predict: Prediction, index: number) => {
              return {
                label: `Person${index}`,
                backgroundColor: color(this.colorList[index])
                  .alpha(0.5)
                  .rgbString(),
                borderColor: color(this.colorList[index]).rgbString(),
                pointBackgroundColor: color(this.colorList[index]).rgbString(),
          ...
          this.personImage = await Promise.all(
            predicts.map(async (predict: Prediction, idx: number) => {
              ...
              const ymin: number =
                canvas.height * predict.detection_box[0] - 25;
              const xmin: number = canvas.width * predict.detection_box[1] - 25;
              const ymax: number =
                canvas.height * predict.detection_box[2] + 25;
